...
export function CardStack<T extends CardStackItem>({
  ...
  React.useEffect(() => {
    const handleResize = () => {
      if (typeof window !== "undefined") {
        if (window.innerWidth < cardWidth + 32) {
          setResponsiveWidth(window.innerWidth - 32);
        } else {
  ...
  const responsiveHeight = Math.round(cardHeight * (responsiveWidth / cardWidth));
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(280, responsiveHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
